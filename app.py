...
Original file is located at
    https://colab.research.google.com/drive/1pb2ieL2P-GfTZ5dvTd6-9mxMC5Neqr3P
...
projectRoot = "C:/Users/kkass/Desktop/Polimi/Laurea/ASP/ProjectAppV3/"
...
iso_reg_death.predict(y_pred_proba_test_death)
...
#print("Shap values number BARC: ", len(shap_values_barc[0]))
...
#+======================================================================================================+
timeEvolutionOutput = html.Div([dcc.Tabs([
...
def update_output(contents, n_clicks, filename):
    ...
    # Parse the data uploaded


    # Analisi
    riskEvolution = {"Entry":[], "RiskEvolution": []}
    pred_proba_test_patient, contribution = ShapModelBarc.predict({'Age': list(temporalEvolution["Age"])[0],  
                                                                   #'Hb': Hb,
    ...
    temporalContrib = pd.DataFrame(contribution, index=[0])
    riskEvolution["RiskEvolution"].append(round(pred_proba_test_patient[0],4))
    riskEvolution["Entry"].append(0)

    for i in range(1,len(temporalEvolution)):
        pred_proba_test_patient, contribution = ShapModelBarc.predict({'Age': list(temporalEvolution["Age"])[i],  
                                                                       #'Hb': Hb,
        ...
        contribution = pd.DataFrame(contribution, index=[0])
        riskEvolution["RiskEvolution"].append(round(pred_proba_test_patient[0],4))
    ...
    return temporalEvolution.to_dict('records'), [{"name": i, "id": i} for i in temporalEvolution.columns],  figRiskEvolution, figFeaturesEvol, fig3D
