...
Original file is located at
    https://colab.research.google.com/drive/1XweseCY_Qxsk14GNoVcJ2FxpN3GZAn3o
...
image_filenameNear = projectRoot + 'logos/near_bianco.png' # replace with your own image
image_filenameY4A = projectRoot + 'logos/U4A_logo_bianco.png' # replace with your own image
...
nearLogo = 'data:image/png;base64,{}'.format(
                                    base64.b64encode(
                                        open(
                                            image_filenameNear, 'rb'
                                        ).read()
                                    ).decode()
                                )
...
def shapScatterCurveFit(data, x, y, colorData, labels, title, fitType, myLevel):
    # Exp Fitting
    if fitType == "exponential":
        ...
        myLevelRisk = linearFunction(myLevel, *popt)
    ...
    plotlyfig.add_trace(go.Scatter(x=np.array([myLevel]*len(data[data[x]==myLevel][y])), y=data[data[x]==myLevel][y],
                                            mode='lines',
                                            name="Risk Range",
...
aboutNear = [html.P("What is NEAR?", className = "typography-question"),
            ...
            html.P("Unlike classic ANN models, NEAR does not require all the input parameters used for training. This flexibility has the price of a loss of reliability in the calculated score proportional to the number of missing values."),
...
aboutTabs = html.Div([dcc.Tabs([
                                    dcc.Tab(label='About the project', children=[*aboutNear,
                                                                                 html.Div([
                                                                                            html.Center(html.Button("🤔 How To Use Near", id="btn_UserGuide", className = "button")),
                                                                                            dcc.Download(id="downloadUserGuide")
                                                                                           ]),
                                                                                ]),
...
titleBox = html.Div(
    ...
    html.Div(children = [
        html.A(
            id='near-logo', children=[
                html.Img(src=nearLogo, className = "y4aLogo")
            ],
                        href="https://www.asp-poli.it/"
                    ),],
    style={"display": "inline-block",
...
predictorsInput = html.Div([
                                          ...
                                          dcc.Markdown('''**Sex assigned at birth:**'''),
                                          dcc.Dropdown(id='sex',
                                                      searchable=False,
                                                      options=[
                                          ...
                                          dcc.Dropdown(id='ethnicity',
                                                      searchable=False,
                                                      options=[
                                                                {'label': 'Caucasian - Other', 'value': 'White'},
                                                                {'label': 'Afrodescendant', 'value': 'Black'}
                                                            ],
                                                      value = 'White'),
...
timeEvolutionInput = html.Div([dcc.Tabs([
                                    ...
                                    dcc.Tab(label='Upload File', children=[html.Div([dcc.Upload(id='datatable-upload',
                                                                                    ...
                                                                                    html.Br(),  
                                                                                    ' Download our ',
                                                                                    html.Button('TEMPLATE 📋', id='btn_template', className = "button"),
                                                                                    dcc.Download(id="downloadTemplate"),
                                                                                    '  to create your dataset, or try our ',
...
#+------------------------------------------------------------------------------------------------------+
#|                    TEMPORAL ANALISYS BARC Update the table and load the plot                         |
#+------------------------------------------------------------------------------------------------------+
#+------------------------------------------------------------------------------------------------------+
#|                    TEMPORAL ANALISYS DEATH Update the table and load the plot                         |
#+------------------------------------------------------------------------------------------------------+


@app.callback(
    Output("downloadUserGuide", "data"),
    Input("btn_UserGuide", "n_clicks"),
...
@app.callback(
    Output("downloadTemplate", "data"),
    Input("btn_template", "n_clicks"),
    prevent_initial_call=True,
)
def func(n_clicks):
    return dcc.send_file(
